...
### B.3 ストレージ肥大化への対策（要検討）

append-only JSONL はセッション単位でしか削除できず、自動クリーンアップが存在しない。
データは `~/.local/state/tmux-agent-log/` 配下に蓄積され、ユーザーが気付きにくい。

**問題**:
1. ユーザーがデータの蓄積場所・量を把握する手段がない
2. `clear` コマンドの存在を知らないと対処できない
3. 長期利用でディスクを静かに消費し続ける

**対策候補（コスト昇順）**:

| 案 | 概要 |
|---|---|
| `status` サブコマンド | セッション数・合計サイズ・最古セッションを表示 |
| `clear` 実行後サマリー | 「3 sessions removed, 12MB freed」のような出力 |
| hook 実行時の通知 | 古いセッションが一定数/容量を超えたら stderr に案内を出す |
| 自動 purge（opt-in） | config で retention 期間を設定、hook 実行時に古いセッションを自動削除 |

**判断時期**: v0.1 リリース後、実際の蓄積ペースを見て優先度を決める。

### B.4 `export` コマンドの UX 見直し（要検討）

現状 `export --session <id>` は個別セッション ID の指定が必須。`list-sessions` → ID コピー → `export` の手順が重い。

**問題**:
1. session ID を事前に知る手段が `list-sessions` しかない
2. 複数セッションをまとめて出力する動線がない
3. 「直近のセッション」「特定 goal のセッション群」など、よくあるパターンへのショートカットがない
...
|---|---|
| `--latest` | 最新セッションを自動選択 |
| `--goal <str>` | 同一 goal のセッションをまとめて出力 |
| `--project <slug>` | プロジェクト単位で出力 |
| `--all` | 全セッションを 1 ファイルに出力 |
| 引数なし = latest | session 未指定時は直近セッションをデフォルトに |

**判断時期**: v0.1 リリース後、実際の利用パターンを見て必要な省略パスを決める。
