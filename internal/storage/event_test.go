...
func TestAppendAndReadEvents(t *testing.T) {
	...
	require.NoError(t, storage.AppendEvent(p, storage.Event{ID: "e1", TurnID: "t1", Ts: time.Unix(1, 0).UTC(), Tool: "Read", Phase: "pre"}))
	require.NoError(t, storage.AppendEvent(p, storage.Event{ID: "e2", TurnID: "t1", Ts: time.Unix(2, 0).UTC(), Tool: "Read", Phase: "post", Success: true}))
