...
6. testify の `require` / `assert` を使ってよい（停止が必要なら require、続行なら assert）
7. 既存の非 table-driven テストは依頼があれば書き換える。無断で巻き込まない
...
外部テスト（`package storage_test`）から `storage.ParseLineRange(...)` で呼べる。
メソッドはエイリアスできないので wrapper 関数を生やす。

**例外**: `cli/hook_test.go` のように cobra `rootCmd` ツリーに直接アクセスする必要があるケースは現状 `package cli` 内部テストを許容。Phase B で `runX(opts)` 関数化が済んだら export_test.go パターンへ揃える。

## 出力モードを混ぜる場合の許容

assert が「exact bytes」と「部分一致」の 2 系統あるとき、構造体に両フィールドを持たせて sentinel で切り替えるのは許容（例: `format/format_test.go::TestWrite` の `want` / `wantContains`）。3 系統以上になったら分離する。
