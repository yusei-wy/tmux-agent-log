...
## private 関数を直接テストすべきか（`export_test.go` の前に問う）

順に問う:

1. **public エントリポイント経由で同じ確信が得られるか？**
   - 得られるなら public 経由で書く（`export_test.go` 不要）
2. **境界値・エッジケースが public 出力に埋もれて検証しづらいか？**
   - パーサ・フォーマッタ・byte-level 契約等は直接テストの価値が高い
3. ↑ で「直接テストが必要」と判断した場合のみ `export_test.go` で公開

「private なら `export_test.go`」ではない。public で書ける限り public で書く。
